...
export async function migrateJsonToSupabase() {
  ...
  try {
    ...
    if (data.clinics && data.clinics.length > 0) {
      ...
      const { error } = await supabase.from('clinics').upsert(clinicsToInsert, { onConflict: 'id' });
      if (error) console.error('[Migration] Error migrating clinics:', error.message);
      else console.log(`[Migration] Migrated ${clinicsToInsert.length} clinics successfully.`);
    }
    ...
    if (data.doctors && data.doctors.length > 0) {
      ...
      const { error } = await supabase.from('doctors').upsert(doctorsToInsert, { onConflict: 'id' });
      if (error) console.error('[Migration] Error migrating doctors:', error.message);
      else console.log(`[Migration] Migrated ${doctorsToInsert.length} doctors successfully.`);
    }
    ...
    if (data.patients && data.patients.length > 0) {
      ...
      const { error } = await supabase.from('patients').upsert(patientsToInsert, { onConflict: 'id' });
      if (error) console.error('[Migration] Error migrating patients:', error.message);
      else console.log(`[Migration] Migrated ${patientsToInsert.length} patients successfully.`);
    }
    ...
    if (data.visits && data.visits.length > 0) {
      ...
      if (error) console.error('[Migration] Error migrating visits:', error.message);
      else console.log(`[Migration] Migrated ${visitsToInsert.length} visits successfully.`);
    }
    ...
    if (data.visit_medications && data.visit_medications.length > 0) {
      ...
      const { error } = await supabase.from('visit_medications').upsert(medsToInsert, { onConflict: 'id' });
      if (error) console.warn('[Migration] Notice for visit_medications:', error.message);
      else console.log(`[Migration] Migrated ${medsToInsert.length} medications successfully.`);
    }
    ...
    if (data.payments && data.payments.length > 0) {
      ...
      const { error } = await supabase.from('payments').upsert(paymentsToInsert, { onConflict: 'id' });
      if (error) console.warn('[Migration] Notice for payments:', error.message);
      else console.log(`[Migration] Migrated ${paymentsToInsert.length} payments successfully.`);
    }
    ...
    if (data.appointments && data.appointments.length > 0) {
      ...
      const { error } = await supabase.from('appointments').upsert(appointmentsToInsert, { onConflict: 'id' });
      if (error) console.warn('[Migration] Notice for appointments:', error.message);
      else console.log(`[Migration] Migrated ${appointmentsToInsert.length} appointments successfully.`);
    }
